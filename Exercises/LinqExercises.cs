...
public sealed class LinqExercises
{
    ...
    public IEnumerable<string> Task01_StudentsFromWarsaw()
    {
        return UniversityData.Students
            ...
            .Select(e => $"{e.IndexNumber} {e.FirstName} {e.LastName} {e.City}")
            .ToList();
    }
        ...
        return new List<string>
    {
        UniversityData.Courses
            ...
            .Select(e => $"{e.Title} {e.StartDate}")
            .FirstOrDefault() ?? "No Analytics course found"
    };
    }
    ...
    {
        return UniversityData.Enrollments.Any(e=>!e.IsActive) ? new List<string>{"Yes"} : new List<string>{"No"};
    }
    ...
    {
        return UniversityData.Lecturers.All(l=>!string.IsNullOrEmpty(l.Department)) ? new List<string>{"Yes"} : new List<string>{"No"}; 
    }
    ...
    {
        return new List<string>{UniversityData.Enrollments.Count(e=>e.IsActive).ToString()};
    }
    ...
    {
        return UniversityData.Enrollments.OrderByDescending(e=>e.EnrollmentDate).Take(3).Select(e=>$"{e.EnrollmentDate} {e.StudentId} {e.CourseId}").ToList();
    }
    ...
    {
        return UniversityData.Courses.OrderBy(c=>c.Title).Skip(2).Take(2).Select(e=>$"{e.Title} {e.Category}").ToList();
    }
    ...
    {
        return UniversityData.Students.Join(UniversityData.Enrollments, s=>s.Id, e=>e.StudentId, (s,e)=>$"{s.FirstName} {s.LastName} {e.EnrollmentDate}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Students, e=>e.StudentId, s=>s.Id, (e,s)=>new {s.FirstName, s.LastName, e.CourseId})
            .Join(UniversityData.Courses, se=>se.CourseId, c=>c.Id, (se,c)=>$"{se.FirstName} {se.LastName} {c.Title}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Courses, e=>e.CourseId, c=>c.Id, (e,c)=>new {c.Title})
            .GroupBy(ec=>ec.Title)
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        return UniversityData.Enrollments.Where(e=>e.FinalGrade!=null)
            ...
            .GroupBy(ec=>ec.Title)
            .Select(g=>$"{g.Key} {g.Average(ec=>ec.FinalGrade)}").ToList();
     }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            ...
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName} {g.Max(se => se.FinalGrade)}")
            .ToList();
        }
    ...
    public IEnumerable<string> Challenge02_AprilCoursesWithoutFinalGrades()
    {
        return UniversityData.Courses
            ...
            .GroupJoin(
                ...
                c => c.Id,
                e => e.CourseId,
                (c, enrollments) => new { c.Title, HasGrades = enrollments.Any(e => e.FinalGrade != null) }
            )
            .Where(cg => !cg.HasGrades)
            .Select(cg => cg.Title)
            .ToList();
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        return UniversityData.Lecturers
            .GroupJoin(
                ...
                l => l.Id,
                c => c.LecturerId,
                (l, courses) => new { l.FirstName, l.LastName, Courses = courses }
            )
            .Select(lc => new
            {
                lc.FirstName,
                lc.LastName,
                AverageGrade = lc.Courses
                    .Join(
                        UniversityData.Enrollments.Where(e => e.FinalGrade != null),
                        c => c.Id,
                        e => e.CourseId,
            ...
            .Select(lg => $"{lg.FirstName} {lg.LastName} {lg.AverageGrade}")
            .ToList();
    ...
    public IEnumerable<string> Challenge04_CitiesAndActiveEnrollmentCounts()
    {
        return UniversityData.Students
            ...
            .GroupBy(city => city)
            .Select(g => new { City = g.Key, ActiveEnrollmentCount = g.Count() })
            .OrderByDescending(c => c.ActiveEnrollmentCount)
            .Select(c => $"{c.City} {c.ActiveEnrollmentCount}")
            .ToList();
        
    }

    //advanced exercises are below:
    //1.Students with more than one active enrollment.
    public IEnumerable<string> Challenge01()
    {
        return UniversityData.Students
            .Join(
                UniversityData.Enrollments.Where(e => e.IsActive),
    ...
    public IEnumerable<string> Challenge02()
    {
        return UniversityData.Courses
            .Where(c => c.StartDate.Month == 4 && c.StartDate.Year == 2026)
            .GroupJoin(
    ...
    //3.Lecturers and average grade across their courses.
    public IEnumerable<string> Challenge03()
    {
        return UniversityData.Lecturers
            .GroupJoin(
                UniversityData.Courses,
    ...
    //4.Cities and number of active enrollments.
    public IEnumerable<string> Challenge04()
    {
        return UniversityData.Students
            .Join(
                UniversityData.Enrollments.Where(e => e.IsActive),
